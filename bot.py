...
def handle_start_help(message):
    if message.chat.type == 'private':
        ...
        bot.send_message(message.chat.id, 'Привет, я бот для игры в бункер. \nТы уже добавил меня в чат, по этому введи команду /reg_start для старта регистрации и /reg_end или /game_start для начала игры.\nДля отмены регистрации можешь ввести /reg_cancel ')


@bot.message_handler(commands=['game_start', 'reg_end'])
def game_start(message):
    if message.chat.type == 'private':
        bot.send_message(message.chat.id, "Ты не можешь начать игру в личных сообщениях, для этого тебе стоит создать группу и добавить туда бота. Остальная информация по команде /help")
    else:
...
def on_message(message):
    ...
    if reroll_needed:
        ...
        bot.send_message(message.chat.id, 'Изменения отправлены в личные сообщения!', reply_markup=remove_keyboard)
        if message.text == 'Персонаж':
            ...
            bot.send_message(message.from_user.id, f'*Ваше измененное состояние здоровья*:\n'
                                                   f'*Состояние здоровья*: {user_health}', markdown, parse_mode="Markdown")
